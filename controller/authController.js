...
exports.signup = async (req, res) => {
  try {
    ...
    // Sending the created token to user after signup
...
exports.login = async (req, res, next) => {
  ...
  if (!email || !password) {
    // Here we will use our AppError.js instead of our own error
  ...
  const user = await User.findOne({ email }).select("+password"); // We use("+password") to include password because we have disable password in userModel.js using select:false
...
// Protecting Tour Routes(Give access to protected routes if user Logged In)
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      res.locals.user = currentUser; // We will have access to this "user" variable in our pug templates
      return next();
    } catch (err) {
      return next();
    }
...
// Authorization: To manage the user role of interacting with certain permissions of API
